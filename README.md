...
![Version](https://img.shields.io/badge/version-5.0.2-596d5c?style=flat-square)
...
> 从 4.4.1 等旧安装版升级时，请使用 5.0.2 Setup；它包含 5.0.1 的安装迁移保护，会先校验、备份并迁移旧安装目录数据，再执行升级。

| 文件 | 用途 |
|---|---|
| `Sanxing-Desktop-5.0.2-setup.exe` | **推荐**，Windows 一键安装版，支持软件内自动更新 |
| `Sanxing-Desktop-5.0.2-win-x64.zip` | 解压即用的完整绿色版 |
| `Sanxing-Desktop-5.0.2-portable.exe` | 单文件备用版，适合临时或隔离使用 |
...
- 首页折叠思维链与思维网思想星体显示整链所有节点的去重关联对象数；
- 展开后的具体节点与回顾卡显示当前节点的局部关联对象数；
- 数字与右侧链接工作区使用同一关联列表，自动过滤删除或失效目标；
- 同一对象被多个节点关联时，整链只计一次，工作区保留关系处数与涉及节点数；
- 数量为 0 时隐藏数字，仅保留链接入口；
- 34/34 自动化与真实浏览器、Portable 隔离启动验证通过。
